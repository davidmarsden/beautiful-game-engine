...
function player(id, clubId, age = 25) {
  return {
    ...
    current_club_id: clubId ? String(clubId) : "",
    current_club: clubId ? `Club ${clubId - 99}` : "Without Club",
    assignment_status: clubId ? "assigned" : "unsigned",
    age
  };
}

test("builds a valid 80-club, four-division world foundation", () => {
  const world = buildWorldFoundation({
    clubUniverse: clubUniverse(),
    gamePlayers: [player(1, 100), player(2, 101, 20)],
    unsignedPlayers: [player(3, null)],
    generatedAt: "2026-07-14T00:00:00.000Z"
  });

  assert.equal(world.clubs.length, 80);
  assert.equal(world.divisions.length, 4);
  assert.ok(world.divisions.every((division) => division.club_capacity === 20));
  assert.equal(world.manager_slots.length, 80);
  assert.equal(world.competitions.length, 5);
  assert.equal(world.season.calendar.league_matchdays.length, 38);
